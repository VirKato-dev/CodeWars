    ...
    public static void main(String[] args) {
        ...
        while (sc.hasNextLine()) {
            ...
            while (in.hasNextInt()) {
                ...
                if (row.size() == 1) {
                    delta = Integer.compare(x, a);
                }
                if (isSafe && row.size() >= 1) {
                    if (delta == 0 || x - a == 0
                            || (delta < 0 && x - a > 0)
                            || (delta > 0 && x - a < 0)
                            || Math.abs(x - a) > 3) {
                        ...
                        List<Integer> l = new ArrayList<>(row);
                        l.add(x);
                        cause = l.toString() + delta;
                    }
                }
                row.add(x);
            }
            System.out.println(row + " : " + (isSafe ? "Safe" : "Unsafe") + " " + cause);
            if (isSafe) {
                result++;
            } else {
                matrix.add(row);
        ...
        for (List<Integer> row : matrix) {
            Set<Integer> set = new LinkedHashSet<>(row);
            int count = row.size() - set.size();
            boolean dub = false;
            if (count > 0) {
                dub = true;
            }

            row = new ArrayList<>(set);
...
//            }
            ...
            // вырезать число нарушающее порядок, если оно одно
            if (dUp != dDown && (dUp == 1 || dDown == 1)) {
                pos = -1;
                for (int i = 1; i < row.size(); i++) {
                    if (Integer.compare(row.get(i), row.get(i - 1)) != delta) {
                        pos = i - 1;
                        if (delta > 0 && row.get(i) < row.get(i - 1)) {
                            pos = i;
                        }
                        break;
                    }
            ...
            System.out.println(row + " (direction:" + delta + ")" + " [double:" + dub + "] " + count + " : " + (count == 1 ? "now safe" : "still bad"));
        }
