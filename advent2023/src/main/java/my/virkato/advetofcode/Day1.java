...
public class Day1 {
    ...
    public static void main(String[] args) {
        Scanner in = new Scanner(Day1Input.input);
        long result = 0;
        while (in.hasNextLine()) {
            result += findNum(in.nextLine());
        }
        System.out.println(result);
    }

    private static int findNum(String line) {
        int left = 0, right = line.length() - 1;
        int foundLeft = -1, foundRight = -1;
        while (!(left > right) && (foundLeft < 0 || foundRight < 0)) {
            if ((foundLeft = checkPosition(line.substring(left))) < 1) {
                left++;
            }
            if ((foundRight = checkPosition(line.substring(right))) < 1) {
                right--;
            }
        }
        return foundLeft * 10 + foundRight;
    }

    private static int checkPosition(String subString) {
        for (int j = 0; j < num.length; j++) {
            if (subString.startsWith(num[j])) {
                return (j + 10) % 10;
            }
        }
        return -1;
