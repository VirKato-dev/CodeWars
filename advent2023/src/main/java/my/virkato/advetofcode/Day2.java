package my.virkato.advetofcode;
...
public class Day2 {
    ...
    public static void main(String[] args) {
        Scanner in = new Scanner(Day2Input.input);
        int result = 0;
        while (in.hasNextLine()) {
//            result += isPossible(in.nextLine());
            result += getPowerOfGame(in.nextLine());
        }
    ...
    private static int isPossible(String line) {
        ...
        int result = Integer.parseInt(line.substring(5, colon));
        String[] sets = line.substring(colon + 1).split(";");
        for (String set : sets) {
            String[] cubes = set.split(",");
            for (String cub : cubes) {
                String[] pair = cub.trim().split(" ");
                if (pair[1].equals("red")) {
                    if (red < Integer.parseInt(pair[0])) return 0;
                } else if (pair[1].equals("blue")) {
                    if (blue < Integer.parseInt(pair[0])) return 0;
                } else if (pair[1].equals("green")) {
                    if (green < Integer.parseInt(pair[0])) return 0;
                }
            }
        }
        return result;
