...
public class Day3 {
    public static void main(String[] args) {
        System.out.println();
        Scanner in = new Scanner(Day3Input.input);
        ...
        System.out.println(sumMarked(sb.toString(), width, height));
    }

    private static long sumMarked(String input, int width, int height) {
        long result = 0;
        for (int row = 0; row < height; row++) {
            String txt = input.substring(row * width, row * width + width);
//            System.out.print(txt + " ");
            int tmp = 0;
            for (int j = 0; j < width; j++) {
                char c = input.charAt(row * width + j);
                if (c >= '0' && c <= '9') {
                    tmp = tmp * 10 + (c - '0');
                    hasMark(input, width, height, row, j);
                } else {
                    if (has) {
                        result += tmp;
                        txt = txt.replace(Integer.toString(tmp), ".".repeat(Integer.toString(tmp).length()));
//                        System.out.print(tmp + "" + hasL + " ");
                    }
                    has = false;
                    tmp = 0;
                }
    ...
    private static void hasMark(String input, int width, int height, int y, int x) {
//        System.out.print("(" + y + "," + x + ") ");
        if (has) return;
        for (int[] ints : near) {
            if (y + ints[0] >= 0 && y + ints[0] < height
                    && x + ints[1] >= 0 && x + ints[1] < width) {
                char c = input.charAt((y + ints[0]) * width + x + ints[1]);
                if (c != '.' && (c < '0' || c > '9')) {
                    has = true;
                    hasL = c;
//                    System.out.print("{" + (y + ints[0]) + "," + (x + ints[1]) + "}");
                    return;
                }
            }
        }
